...
private:
    ...
    */
	...
	void readWordTo(std::string& result)
	{
		...
		if (cur == EOFC)
			throw ReadCheckerException(RR_WF, "Get end of file, expected word token");

		if (isBlanks(cur))
			throw ReadCheckerException(RR_WF, "Get blank spaces, expected word token");
		...
		if (result.length() == 0)
			throw ReadCheckerException(RR_WF, "Get blank spaces or end of file, expected word token");
	}
	...
	*/
	...
	int readInteger()
	{
		...
		long long value = stringToLongLong(_tmpReadToken.c_str());
		if (value < INT_MIN || value > INT_MAX)
			throw ReadCheckerException(RR_WF, "Get " + _tmpReadToken + "  - expected int32");
	...
	int readUnsignedInteger()
	{
		...
		if (_tmpReadToken.size() > 0 && _tmpReadToken[0] == '-')
		{
			std::ostringstream out;
			out << "Get " << _tmpReadToken << ", expected unsigned int";
			throw ReadCheckerException(RR_WF, out.str());
		}

		long long value = stringToLongLong(_tmpReadToken.c_str());
		if (!(value > 0 || value < UINT_MAX))
			throw ReadCheckerException(RR_WF, "Get " + _tmpReadToken + "  - expected unsigned int");

		if (value > UINT_MAX)
		{
			std::ostringstream out;
			out << value << " > " << UINT_MAX << ", expected value in unsigned int's range";
			throw ReadCheckerException(RR_WF, out.str());
		}
	...
	long long readLong()
	{
		...
		return stringToLongLong(_tmpReadToken.c_str());
	}
